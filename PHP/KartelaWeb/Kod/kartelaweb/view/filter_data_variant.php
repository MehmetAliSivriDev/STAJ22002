<?php
    session_start();
    $_SESSION["PAGE"] = "FILTER_DATA_VARIANT";
...
<?php
    ...
    if(isset($_GET["patternCode"]) && isset($_GET["type"]) && isset($_GET["pattern"])){
        ...
        // SQL sorgusu 
        $sql = "SELECT DISTINCT Varyant FROM KartelaData WHERE DesenKod = :v1 AND Tip = :v2 AND Desen = :v3";
...
<head>
    ...
    <title>Veri Filtreleme(Varyant)</title>
...
<body>
    ...
    <div class="container-fluid text-center">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6">  
                <form action="/kartelaweb/backend/filter.php" method="post">
                    <input type="hidden" name="filter" value="pattern">
        ...
        <form action="filter_data_color.php" method="GET">
            <input type="hidden" name="patternCode" value="<?php echo $patternCode?>">
            <input type="hidden" name="type" value="<?php echo $type?>">
            <input type="hidden" name="pattern" value="<?php echo $pattern?>">
            <?php 
                foreach ($variants as $variant) {?>
                    <button type="submit" class="btn btn-outline-secondary m-1 p-2 container-lg" name="variant" value="<?php echo $variant["Varyant"]?>">
                    <?php echo $variant["Varyant"]?></button><br>
            <?php } ?>
        </form>
