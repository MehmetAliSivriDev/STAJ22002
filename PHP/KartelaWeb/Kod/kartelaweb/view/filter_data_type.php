<?php
    session_start();
    $_SESSION["PAGE"] = "FILTER_DATA_TYPE";
...
<?php
    ...
    if(isset($_GET["patternCode"])){
        
        $patternCode = test_input($_GET["patternCode"]);

        // SQL sorgusu 
        $sql = "SELECT DISTINCT Tip FROM KartelaData WHERE DesenKod = :v1";
...
<head>
    ...
    <title>Veri Filtreleme(Tip)</title>
...
<body>
    ...
    <div class="container-fluid text-center">  
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6">  
                <form action="/kartelaweb/backend/filter.php" method="POST">
                    <input type="hidden" name="filter" value="patternCode">
        ...
        <form action="filter_data_pattern.php" method="GET">
            <input type="hidden" name="patternCode" value="<?php echo $patternCode?>">
            <?php 
                foreach ($types as $type) {?>
                    <button type="submit" class="btn btn-outline-secondary m-1 p-2 container-lg" name="type" value="<?php echo $type["Tip"]?>">
                    <?php echo $type["Tip"]?></button><br>
            <?php } ?>
        </form>
